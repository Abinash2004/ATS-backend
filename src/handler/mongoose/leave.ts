...
import {normalizeDate, parseDateDMY} from "../../utils/date_time.ts";

const LEAVE_TTL = 60 * 60 * 1000;
const leaveKey = (date: string | Date, employeeId: string) => `leave:${normalizeDate(date)}:${employeeId}`;
...
async function getApprovedLeave(leaveDate: Date, employeeId: string): Promise<ILeave | null> {
    try {
        ...
        const leaveCache = await redisClient.get(leaveKey(normalizedDate, employeeId));
        if (leaveCache) return JSON.parse(leaveCache);
        const leaveData = await Leave.findOne({date: leaveDate,employeeId,leave_status: "approved"}).lean<ILeave>().exec();
        await redisClient.set(leaveKey(normalizedDate, employeeId),JSON.stringify(leaveData),{ EX: LEAVE_TTL });
        return leaveData;
    } catch(error) {
        console.log(error);
        return null;
